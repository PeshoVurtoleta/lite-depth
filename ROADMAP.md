...
| **D-10** | S3 | **No torture entry point.** The suite law is "every module change is proven by `node --expose-gc test/torture.mjs`". This package has no `test/torture.mjs`, no `torture` script, and no `prepublishOnly` gate. Its zero-GC claims rest on two `node:test` files that skip without `--expose-gc`. | `ls test/`, `package.json:33` |
| **D-11** | S3 | **Docs-vs-tree drift in the test count.** llms.txt line 163 claims "9 files: core 01-04, motion 05-09". The tree has 5: `01-math`, `03-pipeline`, `04-zero-gc`, `06-motion-quaternion`, `09-motion-zero-gc`. Files 02, 05, 07, 08 do not exist. Either four suites were lost or the number was aspirational; both are docs the pipeline reads when planning. | `ls test/` -> 5 files |
...
| **D-13** | S3 | **`stats` cannot see the failure modes above.** No `facesOverflowed`, no `nodesInvalid`, no `nodesTotal`. `stage._order` / `stage._drawCount` are underscore-private and asserted against by tests -- publish them or freeze them, but a test pinning a private is a contract nobody wrote down. | `Depth.js:334`, `566` |
